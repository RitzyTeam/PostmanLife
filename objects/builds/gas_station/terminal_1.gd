...
func buy_petrol():
	...
	var petrol_left: int = gas_station.petrol_station_1
	if petrol_left >= buy_amount:
		var petrol_price: int = buy_amount * gas_station.petrol_price_station_1
		if SIN_WORLD_DATA.WORLD_DATA['money'] >= petrol_price:
			...
			gas_station.petrol_station_1 -= buy_amount
	else:
		if petrol_left > 0:
			var petrol_price: int = gas_station.petrol_station_1 * gas_station.petrol_price_station_1
			if SIN_WORLD_DATA.WORLD_DATA['money'] >= petrol_price:
				isSuccess = true
				amountBought = gas_station.petrol_station_1
				SIN_WORLD_DATA.WORLD_DATA['money'] -= petrol_price
				SIN_WORLD_SIGNALS.emit_signal('PLAYER_UI_CASH_UPDATE', -petrol_price)
				gas_station.petrol_station_1 = 0
	if isSuccess:
